...
// 参考文档
// https://www.jianshu.com/p/d71302690af2
...
func DesEncrypt(data, key []byte, desType adesconst.DesType, mode adesconst.EncryptMode, padMode adesconst.PaddingMode) (out []byte, err error) {
	...
	if desType == adesconst.Des {
		if len(key) != 8 {
			err = errors.New("秘钥长度不为8")
			return
		}
	...
	} else if desType == adesconst.TripleDes {
		if len(key) != 24 {
			err = errors.New("秘钥长度不为24")
			return
		}
